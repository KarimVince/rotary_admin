...
def test_admin_can_create_user(admin_client):
    response = admin_client.post("/api/users", json=_create_payload())
...
def test_non_admin_cannot_create_user(user_client):
    response = user_client.post("/api/users", json=_create_payload())

    assert response.status_code == 403


def test_treasurer_cannot_create_user(treasurer_client):
    response = treasurer_client.post("/api/users", json=_create_payload())

    assert response.status_code == 403


def test_create_user_duplicate_email_returns_409(admin_client):
    admin_client.post("/api/users", json=_create_payload())
...
def test_admin_can_change_user_role(admin_client):
    created = admin_client.post("/api/users", json=_create_payload()).json()

    response = admin_client.patch(f"/api/users/{created['id']}", json={"role": "treasurer"})
...
def test_patch_nonexistent_user_returns_404(admin_client):
    response = admin_client.patch(
        "/api/users/00000000-0000-0000-0000-000000000000", json={"is_active": False}
    )
