...
def test_create_then_list_member_title_round_trip(admin_client):
    create_response = admin_client.post(
        "/api/member-titles", json={"code": "Rtn", "label": "Rotarian", "sort_order": 0}
    )
    ...
    list_response = admin_client.get("/api/member-titles")
...
def test_list_member_titles_requires_authentication(client):
    response = client.get("/api/member-titles")
...
def test_non_admin_cannot_create_member_title(user_client):
    response = user_client.post(
        "/api/member-titles", json={"code": "PP", "label": "Past President", "sort_order": 1}
    )
...
def test_treasurer_cannot_create_member_title(treasurer_client):
    response = treasurer_client.post(
        "/api/member-titles", json={"code": "IPP", "label": "Immediate Past President"}
    )
