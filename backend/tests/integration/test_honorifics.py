...
def test_create_then_list_honorific_round_trip(admin_client):
    create_response = admin_client.post(
        "/api/v1/honorifics", json={"code": "ZDR", "label": "Dr.", "sort_order": 0}
    )
    assert create_response.status_code == 201
    created = create_response.json()
    assert created["code"] == "ZDR"
...
def test_non_admin_cannot_create_honorific(user_client):
    response = user_client.post(
        "/api/v1/honorifics", json={"code": "ZMR", "label": "Mr.", "sort_order": 0}
    )
...
def test_treasurer_cannot_create_honorific(treasurer_client):
    response = treasurer_client.post("/api/v1/honorifics", json={"code": "ZMS", "label": "Ms."})

    assert response.status_code == 403


def test_create_honorific_duplicate_code_returns_409(admin_client):
    admin_client.post("/api/v1/honorifics", json={"code": "ZMRS", "label": "Mrs."})

    response = admin_client.post("/api/v1/honorifics", json={"code": "ZMRS", "label": "Duplicate"})

    assert response.status_code == 409


def test_admin_can_update_honorific(admin_client):
    created = admin_client.post("/api/v1/honorifics", json={"code": "ZPROF", "label": "Prof"}).json()
...
def test_update_honorific_duplicate_code_returns_409(admin_client):
    admin_client.post("/api/v1/honorifics", json={"code": "ZMR", "label": "Mr."})
    other = admin_client.post("/api/v1/honorifics", json={"code": "ZMS", "label": "Ms."}).json()

    response = admin_client.patch(f"/api/v1/honorifics/{other['id']}", json={"code": "ZMR"})
...
def test_non_admin_cannot_update_honorific(admin_client, user_client):
    created = admin_client.post("/api/v1/honorifics", json={"code": "ZMISS", "label": "Miss"}).json()
...
def test_non_admin_cannot_deactivate_honorific(admin_client, user_client):
    created = admin_client.post("/api/v1/honorifics", json={"code": "ZDR", "label": "Dr."}).json()
...
def test_admin_can_deactivate_honorific(admin_client):
    created = admin_client.post("/api/v1/honorifics", json={"code": "ZMR", "label": "Mr."}).json()
...
def test_deactivating_honorific_does_not_break_members_referencing_it(admin_client):
    honorific = admin_client.post("/api/v1/honorifics", json={"code": "ZDR", "label": "Dr."}).json()
