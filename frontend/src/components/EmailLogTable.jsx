...
const STATUS_STYLES_MINIMAL = {
  ...
  no_recipients: { bgClass: "bg-[var(--bg-alt)]", textClass: "text-[var(--muted)]", label: "No recipients" },
};
          ...
          <tbody>
            ...
            {entries.map((entry) => (
              <tr key={entry.id} className="border-b border-[var(--color-border-light)] last:border-b-0">
                <td
                  title={entry.subject}
                  className="max-w-0 truncate px-4 py-2 text-[13px] font-semibold text-[var(--text-h)]"
                >
                  {entry.subject}
                </td>
