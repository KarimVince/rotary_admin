...
- `POST /api/v1/members/email/attachments` — admin-only, uploads one file (size-capped via `MAX_EMAIL_ATTACHMENT_BYTES`, default 10MB) and returns `{filename, url}`. Sender's API takes attachments as `{filename: publicly_accessible_url}` — it fetches the file itself rather than accepting raw bytes — so the URL must be reachable from the internet (`PUBLIC_BASE_URL` setting). Front-load one or more of these before calling `/members/email` with an `attachments` array; `email_log.has_attachments` records whether a send included any.
